...
function ftsDelete(db, id) {
    const row = db.prepare('SELECT rowid FROM kb_fts WHERE rowid = ?').get(id);
    if (row === undefined)
        return;
...
function syncFts(db) {
    const dataCount = db.prepare('SELECT COUNT(*) AS n FROM kb').get().n;
    const ftsCount = db.prepare('SELECT COUNT(*) AS n FROM kb_fts').get().n;
    if (dataCount === ftsCount)
        return;
 ...
 * @returns 条目 id。
 */
export function upsertEntry(db, input) {
...
export function updateEntry(db, id, category, tags) {
...
export function listEntries(db, category) {
 ...
 */
export function searchKb(db, args) {
    ...
    if (q.length >= 3) {
        try {
          ...
          ORDER BY bm25(kb_fts)
          LIMIT 50`;
            const params = category !== undefined ? [category, match] : [match];
 ...
 * @returns 受影响条目数。
 */
export function moveFile(db, source, category) {
 ...
 * @returns 受影响条目数。
 */
export function renameFile(db, source, newSource) {
...
/** 读取用户手动新建的分类列表（含空目录）。 */
export function getCustomCategories(db) {
...
export function removeCategory(db, name) {
    const row = db.prepare('SELECT COUNT(*) AS n FROM kb WHERE category = ?').get(name);
    if (row.n > 0)
        return false;
...
export function renameCustomCategory(db, oldName, newName) {
    const list = getCustomCategories(db);
...
export function deleteFile(db, source) {
