...
import { kbRoot, openKb, upsertEntry, updateEntry, listEntries, searchKb, renameCategory, renameCustomCategory, moveFile, renameFile, addCategory, removeCategory, getCustomCategories, deleteFile, deleteEntry, } from "./kb.js";
import { parseDocument, suggestCategory } from "./parse.js";
...
export function apply(ctx, config) {
        ...
        handler: async (req, res) => {
            try {
                ...
                if (!Number.isFinite(id) || category === '')
                    return sendJson(res, 400, { error: 'id 与 category 必填' });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            ...
            if (q === '')
                return sendJson(res, 400, { error: 'q 必填' });
        ...
        handler: async (req, res) => {
            try {
                ...
                if (oldCategory === '' || newCategory === '')
                    return sendJson(res, 400, { error: 'oldCategory 与 newCategory 必填' });
                if (oldCategory === newCategory)
                    return sendJson(res, 200, { renamed: 0 });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                if (name === '')
                    return sendJson(res, 400, { error: 'name 必填' });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                if (!removed)
                    return sendJson(res, 400, { error: '目录下有条目，无法删除（请先移动或清空）' });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                const category = String(body.category ?? '');
                if (source === '' || category === '')
                    return sendJson(res, 400, { error: 'source 与 category 必填' });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                if (source === '' || newSource === '')
                    return sendJson(res, 400, { error: 'source 与 newSource 必填' });
                if (source === newSource)
                    return sendJson(res, 200, { renamed: 0 });
    ...
    webServer.register({
        ...
        handler: async (req, res) => {
            try {
                ...
                const source = String(body.source ?? '');
                if (source === '')
                    return sendJson(res, 400, { error: 'source 必填' });
        ...
        handler: async (req, res) => {
            try {
                ...
                if (!Number.isFinite(id))
                    return sendJson(res, 400, { error: 'id 必填' });
                deleteEntry(db, id);
    ...
    webServer.register({
        ...
        handler: async (_req, res) => {
            const rows = db.prepare('SELECT source, category, COUNT(*) AS n, MAX(updated_at) AS t FROM kb GROUP BY source ORDER BY t DESC').all();
        ...
        handler: async (req, res) => {
            try {
                ...
                const targets = sources !== undefined
                    ...
                    : db.prepare("SELECT source FROM kb GROUP BY source HAVING category = '未分类' ORDER BY MAX(updated_at) DESC LIMIT 20").all();
...
function allCategories(db, configCategories) {
    ...
    for (const c of getCustomCategories(db))
        set.add(c);
...
function collectFileContent(db, source) {
    const rows = db.prepare('SELECT payload FROM kb WHERE source = ? ORDER BY id LIMIT 5').all(source);
