...
import { openKb, upsertEntry, updateEntry, deleteEntry, listEntries, searchKb, type KbSearchArgs } from './kb.ts'
...
export function apply(ctx: Context, config: Config): (() => void) | void {
  ...
  ctx.tools.register(defineTool({
    name: 'kb_query',
    description:
      '检索知识库（kb = Knowledge Base，知识库）。按关键词在名称/摘要/内容/标签上匹配，' +
      '可按分类过滤。知识库内容来自 kb_import 或 Web 界面导入的文档。' +
      '典型用法：回答业务问题前先查知识库是否有相关资料。',
    parameters: {
    ...
    async execute(args: KbSearchArgs) {
      const rows = searchKb(db, args)
      if (rows.length === 0) {
        return JSON.stringify({ matched: 0, hint: `知识库中未找到与 "${args.query}" 相关的内容` })
      }
  ...
  ctx.tools.register(defineTool({
    name: 'kb_import',
    description:
      '导入本机文件到知识库（支持 md/txt/json/yml/docx/pdf）。' +
      '文件按标题/段落自动切块，每块成为一条可检索条目；同名文件重新导入 = 覆盖更新。' +
      '导入时给出 category（分类）与 tags（标签）；未指定则自动建议分类。' +
      '典型用法：把产品手册、规范书、认证文件等导入知识库。',
    parameters: {
    ...
    async execute(args: { path: string; category?: string; tags?: string[] }) {
      const parsed = await parseDocument(args.path)
      const suggested = args.category ?? suggestCategory(args.path, parsed.chunks[0]?.payload ?? '', config.categories)
      const category = suggested ?? defaultCategory(config.categories)
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      id: { type: 'number', required: true, description: '条目 id（来自 kb_list / kb_query）' },
      category: { type: 'string', required: true, description: `新分类：${config.categories.join(' / ')}` },
    ...
    async execute(args: { id: number; category: string; tags?: string[] }) {
      updateEntry(db, args.id, args.category, args.tags)
      return JSON.stringify({ updated: args.id, category: args.category })
    },
  }))
