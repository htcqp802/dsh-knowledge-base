...
async function extractPdfWithPdfjs(filePath: string): Promise<string> {
...
export function chunkText(text: string, sourceName: string): Chunk[] {
 ...
 */
function mergeSmallChunks(chunks: Chunk[], sourceName: string): Chunk[] {
 ...
 */
export function suggestCategory(filePath: string, text: string, categories: readonly string[]): string | undefined {
    ...
    class DOMMatrix {
      ...
      constructor(init?: unknown) {
        ...
        if (typeof init === 'string') {
          ...
          if (m !== null) {
            const nums: number[] = (m[1] ?? '').split(',').map((n) => Number(n) || 0)
            if (nums.length === 6) {
              const g = (i: number): number => nums[i] ?? 0
        ...
        if (Array.isArray(init)) {
          if (init.length === 16) {
            ...
            const get = (i: number): number => arr[i] ?? 0
            this.m11 = get(0); this.m12 = get(1); this.m13 = get(2); this.m14 = get(3)
            this.m21 = get(4); this.m22 = get(5); this.m23 = get(6); this.m24 = get(7)
            this.m31 = get(8); this.m32 = get(9); this.m33 = get(10); this.m34 = get(11)
            this.m41 = get(12); this.m42 = get(13); this.m43 = get(14); this.m44 = get(15)
            this.#sync()
          }
          return
        }
        if (typeof init === 'object') {
          ...
          if (o.m11 !== undefined) {
            this.m11 = o.m11 ?? 1; this.m12 = o.m12 ?? 0; this.m13 = o.m13 ?? 0; this.m14 = o.m14 ?? 0
            this.m21 = o.m21 ?? 0; this.m22 = o.m22 ?? 1; this.m23 = o.m23 ?? 0; this.m24 = o.m24 ?? 0
            this.m31 = o.m31 ?? 0; this.m32 = o.m32 ?? 0; this.m33 = o.m33 ?? 1; this.m34 = o.m34 ?? 0
            this.m41 = o.m41 ?? 0; this.m42 = o.m42 ?? 0; this.m43 = o.m43 ?? 0; this.m44 = o.m44 ?? 1
          } else {
      ...
      #set2d(a: number, b: number, c: number, d: number, e: number, f: number): void {
      ...
      set(...args: number[]): this {
        if (args.length >= 6) {
          const g = (i: number): number => args[i] ?? 0
          this.#set2d(g(0), g(1), g(2), g(3), g(4), g(5))
        }
        return this
      }
...
function multiply2d(left: [number, number, number, number, number, number], right: [number, number, number, number, number, number]): [number, number, number, number, number, number] {
